...
let tasks = ["numeral", "ABC", "color"],
	...
	board, key, block, input, limit, seq, delay, wl
...
const audio = new AudioContext(),
	...
	prep = () => {
		...
		;[input, limit] = [[], limit > range.max ? range.min : limit]
	},
	...
	run = (el, stm = block[run.n]) => {
		...
		el.className = ["stimulus", stm, key].join(" ")
		if (sound && dual && key !== tasks.at(-1)) speak(getComputedStyle(el, "::after").content.toLowerCase())
		setTimeout(() => el.classList.remove(stm, key), time.stimulus)
	},
	task = () => {
		prep()
